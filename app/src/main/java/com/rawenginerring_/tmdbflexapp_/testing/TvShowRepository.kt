...
class TvShowRepository(val tvShowRequests: TvShowRequests): TvShowRepositoryI {
    ...
    override fun getTvDetails(id: String): Observable<Quadruple<MovieInfo, Credits, Review, MovieList>> {
        return Observable.create < Quadruple < MovieInfo, Credits, Review, MovieList >> { emitter ->
            val movieInfoCall = tvShowRequests?.getTvShowInfo(id)
            val creditsCall = tvShowRequests?.getCredits(id)
            val reviewsCall = tvShowRequests?.getReviews(id)
            val similarMoviesCall = tvShowRequests?.getSimilar(id)
